...
3. **「URL の内容を取得」** を追加:
   ...
   - **リクエスト本文:** **JSON** を選び、例として次のようにマッピング:
     - キー `text`、値に **「ショートカット入力」**（直前の「いつ・誰と」の回答など）を接続
     - キー **`meet`**、値 **`true`**（**必須に近い**）— 本文に「ミーティング」「打ち合わせ」等が無いと API は **Meet なしのカレンダーのみ**になるため、Siri 用ショートカットでは **`meet: true` を付けて Meet を強制**する

JSON の例（ショートカット内の辞書で表現する場合）:

```json
{ "text": "<ショートカット入力が入る>", "meet": true, "slack_user_id": "Uxxxxxxxx" }
```

（`slack_user_id` は任意。付けると VM の `meet/data/google_tokens/<U>.json` をそのリクエストの Google トークンとして使う。）
...
| Meet URL が付かない（カレンダーだけできる） | 本文に会議キーワードが無いと Meet なし分岐になる。**JSON に `"meet": true`** を付ける（上記） |
