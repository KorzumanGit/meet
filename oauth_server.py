...
def proxy_ios_shortcut_meet():
    ...
    JSON 本文は改変せず転送する。slack_user_id は oauth_server では使わず、
    Node 側が data/google_tokens/<U>.json を読む（本番は meet/.env とボリュームマウントで共有）。
    """
    ...
    if raw_body:
        try:
            ...
            if isinstance(payload, dict):
                sid = payload.get("slack_user_id")
                if isinstance(sid, str) and _SLACK_USER_RE.match(sid.strip()):
                    print(
                        f"POST /api/meet → {url} (slack_user_id={sid.strip()} を Node がトークン解決に使用可能)",
                        flush=True,
                    )
